...
namespace org.apache.hadoop.hive.ql.io.orc
{
     ...
     */
    abstract public class RedBlackTree<T>
    {
        public const int NULL = -1;

        // Various values controlling the offset of the data within the array.
        private const int LEFT_OFFSET = 0;
        private const int RIGHT_OFFSET = 1;
        private const int ELEMENT_SIZE = 2;

        protected int size = 0;
        private DynamicIntArray data;
        protected int root = NULL;
        protected int lastAdd = 0;
        private bool wasAdd = false;
         ...
         */
        private int insert(int left, int right, bool isRed)
        {
            int position = size;
            size += 1;
         ...
         */
        protected abstract int compareValue(int position, T value);
         ...
         */
        private void setRed(int position, bool isRed)
        {
         ...
         */
        protected internal int getLeft(int position)
        {
         ...
         */
        protected internal int getRight(int position)
        {
         ...
         * Note that we are storing the node color in the low bit of the left pointer.
         */
        private void setLeft(int position, int left, bool isRed)
        {
            int offset = position * ELEMENT_SIZE + LEFT_OFFSET;
         ...
         */
        private bool add(T value, int node, bool fromLeft, int parent,
                            int grandparent, int greatGrandparent)
        {
            ...
            {
                int compare = compareValue(node, value);
                bool keepGoing;

                // Recurse down to find where the node needs to be added
                if (compare < 0)
                {
                    keepGoing = add(value, getLeft(node), true, node, parent, grandparent);
                }
                else if (compare > 0)
                {
                    keepGoing = add(value, getRight(node), false, node, parent, grandparent);
                }
                else
                {
                    lastAdd = node;
         ...
         */
        protected bool addBase(T value)
        {
            add(value, root, false, NULL, NULL, NULL);
            if (wasAdd)
            {
                setRed(root, false);
                return true;
