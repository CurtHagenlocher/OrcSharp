...
namespace org.apache.hadoop.hive.ql.io.orc
{
     ...
     * </p>
     */
    class RunLengthIntegerWriterV2 : IntegerWriter
    {
        ...
        public const int MAX_SCOPE = 512;
        public const int MIN_REPEAT = 3;
        private const int MAX_SHORT_REPEAT_LENGTH = 10;
        ...
        private long[] zigzagLiterals = new long[MAX_SCOPE];
        private long[] baseRedLiterals = new long[MAX_SCOPE];
        private long[] adjDeltas = new long[MAX_SCOPE];
        ...
        private void writeDeltaValues()
        {
            ...
            // write header
            output.WriteByte((byte)headerFirstByte);
            output.WriteByte((byte)headerSecondByte);
        ...
        private void writePatchedBaseValues()
        {
            ...
            int fb = brBits95p;
            int efb = utils.encodeBitWidth(fb) << 1;
            ...
            int headerThirdByte = bb | utils.encodeBitWidth(patchWidth);
            ...
            output.WriteByte((byte)headerFirstByte);
            output.WriteByte((byte)headerSecondByte);
            output.WriteByte((byte)headerThirdByte);
            output.WriteByte((byte)headerFourthByte);

            // write the base value using fixed bytes in big endian order
            for (int i = baseBytes - 1; i >= 0; i--)
            {
                byte b = (byte)(((ulong)min >> (i * 8)) & 0xff);
                output.WriteByte(b);
            }
        ...
        private void writeShortRepeatValues()
        {
            ...
            int numBitsRepeatVal = utils.findClosestNumBits(repeatVal);
            int numBytesRepeatVal = numBitsRepeatVal % 8 == 0 ? numBitsRepeatVal >> 3
                : (int)((uint)numBitsRepeatVal >> 3) + 1;
            ...
            {
                int b = (int)(((ulong)repeatVal >> (i * 8)) & 0xff);
        ...
        private void preparePatchedBlob()
        {
            ...
            patchLength = (int)Math.Ceiling((numLiterals * 0.05));
            ...
            for (int i = 0; i < numLiterals; i++)
            {
                ...
                if (baseRedLiterals[i] > mask)
                {
                    ...
                    long patch = (long)((ulong)baseRedLiterals[i] >> brBits95p);
                ...
                else
                {
                    ...
                    if (prevDelta == 0 && currentDelta == 0)
                    {
                        ...
                        if (fixedRunLength >= MIN_REPEAT && variableRunLength > 0)
                        {
                            ...
                            Array.Copy(literals, numLiterals, tailVals, 0, MIN_REPEAT);
