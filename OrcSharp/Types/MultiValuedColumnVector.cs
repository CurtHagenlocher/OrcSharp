...
namespace OrcSharp.Types
{
     ...
     */
    public abstract class MultiValuedColumnVector : ColumnVector
    {
        ...
        protected abstract void childFlatten(bool useSelected, int[] selected, int size);
        ...
        public override void flatten(bool selectedInUse, int[] sel, int size)
        {
            ...
            if (isRepeating)
            {
                if (noNulls || !isNull[0])
                {
                    ...
                    if (offsets[0] != 0)
                    {
                        throw new ArgumentException("Repeating offset isn't 0, but " +
                                                           offsets[0]);
                    }
        ...
        public override void ensureSize(int size, bool preserveData)
        {
            if (size > offsets.Length)
            {
                ...
                if (preserveData)
                {
                    ...
                    {
                        Array.Copy(oldOffsets, 0, offsets, 0, oldOffsets.Length);
                        Array.Copy(oldLengths, 0, lengths, 0, oldLengths.Length);
                    }
                }
            }
        }
