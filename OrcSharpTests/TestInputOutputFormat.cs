...
public class TestInputOutputFormat {
  ...
  public void testOverlap()  {
    Assert.Equal(0, OrcInputFormat.SplitGenerator.getOverlap(100, 100,
        200, 100));
    Assert.Equal(0, OrcInputFormat.SplitGenerator.getOverlap(0, 1000,
        2000, 100));
    Assert.Equal(100, OrcInputFormat.SplitGenerator.getOverlap(1000, 1000,
        1500, 100));
    Assert.Equal(250, OrcInputFormat.SplitGenerator.getOverlap(1000, 250,
        500, 2000));
    Assert.Equal(100, OrcInputFormat.SplitGenerator.getOverlap(1000, 1000,
        1900, 1000));
    Assert.Equal(500, OrcInputFormat.SplitGenerator.getOverlap(2000, 1000,
        2500, 2000));
  }
  ...
  [Fact]
  public void testSplitStrategySelection()  {
  ...
  public void testFileGenerator()  {
    ...
    Assert.Equal(true, splitStrategy instanceof OrcInputFormat.BISplitStrategy);
    ...
    Assert.Equal(true, splitStrategy instanceof OrcInputFormat.ETLSplitStrategy);
  }

  [Fact]
  public void testEtlCombinedStrategy()  {
    ...
    Assert.Equal(1, etlSs.files.size());
    ...
    Assert.Equal(2, rejectedEtlSs.files.size());
    Assert.Equal(1, rejectedEtlSs.dirs.size());
  ...
  public OrcInputFormat.AcidDirInfo createAdi(
      OrcInputFormat.Context context, MockFileSystem fs, String path) {
  ...
  private OrcInputFormat.SplitStrategy createSplitStrategy(
      OrcInputFormat.Context context, OrcInputFormat.FileGenerator gen) {
  ...
  static class MockInputStream extends FSInputStream {
    ...
    @Override
    public bool seekToNewSource(long l) {
      return false;
    }
  ...
  public static class MockOutputStream extends FSDataOutputStream {
    ...
    public void close() {
      ...
      Array.Copy(buf.getData(), 0, file.content, 0, file.length);
    }
  ...
  public static class MockFileSystem extends FileSystem {
    ...
    @Override
    public bool rename(Path path, Path path2) {
      return false;
    }

    @Override
    public bool delete(Path path) {
      return false;
    }

    @Override
    public bool delete(Path path, bool b) {
      return false;
    }
  ...
  static void fill(DataOutputBuffer out, long length) {
   ...
   * @throws IOException
   */
  static byte[] createMockOrcFile(long... stripeLengths) {
    ...
    Array.Copy(buffer.getData(), 0, result, 0, buffer.getLength());
    return result;
  }
  ...
  public void testAddSplit()  {
    ...
    Assert.Equal(0, result.getStart());
    Assert.Equal(200, result.getLength());
    Assert.Equal("mock:/a/file", result.getPath().toString());
    String[] locs = result.getLocations();
    Assert.Equal(3, locs.length);
    Assert.Equal("host1-1", locs[0]);
    Assert.Equal("host1-2", locs[1]);
    Assert.Equal("host1-3", locs[2]);
    result = splitter.createSplit(500, 600, null);
    locs = result.getLocations();
    Assert.Equal(3, locs.length);
    Assert.Equal("host2-1", locs[0]);
    Assert.Equal("host0", locs[1]);
    Assert.Equal("host2-3", locs[2]);
    result = splitter.createSplit(0, 2500, null);
    locs = result.getLocations();
    Assert.Equal(1, locs.length);
    Assert.Equal("host0", locs[0]);
  }
  ...
  public void testSplitGenerator()  {
    ...
    Assert.Equal(3, result.getStart());
    Assert.Equal(497, result.getLength());
    result = results.get(1);
    Assert.Equal(500, result.getStart());
    Assert.Equal(600, result.getLength());
    result = results.get(2);
    Assert.Equal(1100, result.getStart());
    Assert.Equal(400, result.getLength());
    result = results.get(3);
    Assert.Equal(1500, result.getStart());
    Assert.Equal(300, result.getLength());
    result = results.get(4);
    Assert.Equal(1800, result.getStart());
    Assert.Equal(200, result.getLength());
    ...
    for(int i=0; i < stripeSizes.length; ++i) {
      Assert.Equal("checking stripe " + i + " size",
          stripeSizes[i], results.get(i).getLength());
    }
  }

  [Fact]
  public void testProjectedColumnSize()  {
    long[] stripeSizes =
    ...
    Assert.Equal(3, result.getStart());
    Assert.Equal(400, result.getLength());
    Assert.Equal(167468, result.getProjectedColumnsUncompressedSize());
    result = results.get(1);
    Assert.Equal(403, result.getStart());
    Assert.Equal(400, result.getLength());
    Assert.Equal(167468, result.getProjectedColumnsUncompressedSize());
    result = results.get(2);
    Assert.Equal(803, result.getStart());
    Assert.Equal(100, result.getLength());
    Assert.Equal(41867, result.getProjectedColumnsUncompressedSize());
    ...
    for (int i = 0; i < stripeSizes.length; ++i) {
      ...
      if (i == stripeSizes.length - 1) {
        Assert.Equal(41867, results.get(i).getProjectedColumnsUncompressedSize());
      } else {
        Assert.Equal(83734, results.get(i).getProjectedColumnsUncompressedSize());
      }
    }
    ...
    splitter = new OrcInputFormat.SplitGenerator(new OrcInputFormat.SplitInfo(context, fs,
        ...
        true, null, null));
    results = splitter.call();
    Assert.Equal(1, results.size());
    result = results.get(0);
    Assert.Equal(3, result.getStart());
    Assert.Equal(900, result.getLength());
    Assert.Equal(376804, result.getProjectedColumnsUncompressedSize());
  }
  ...
  public void testInOutFormat()  {
    ...
    Assert.Equal(OrcSerde.OrcSerdeRow.class, serde.getSerializedClass());
    inspector = (StructObjectInspector) serde.getObjectInspector();
    Assert.Equal("struct<x:int,y:int>", inspector.getTypeName());
    ...
    Assert.Equal(0.33, reader.getProgress(), 0.01);
    while (reader.next(key, value)) {
      Assert.Equal(++rowNum, intInspector.get(inspector.
          getStructFieldData(serde.deserialize(value), fields.get(0))));
      Assert.Equal(2, intInspector.get(inspector.
          getStructFieldData(serde.deserialize(value), fields.get(1))));
    }
    Assert.Equal(3, rowNum);
    Assert.Equal(1.0, reader.getProgress(), 0.00001);
    reader.close();
    ...
    while (reader.next(key, value)) {
      ...
      Assert.Equal(null, inspector.getStructFieldData(value, fields.get(1)));
    }
    Assert.Equal(3, rowNum);
    reader.close();
  ...
  public void testMROutput()  {
    ...
    IntObjectInspector intInspector =
        (IntObjectInspector) fields.get(0).getFieldObjectInspector();
    while (reader.next(key, value)) {
      Assert.Equal(null, inspector.getStructFieldData(value, fields.get(0)));
      Object sub = inspector.getStructFieldData(value, fields.get(1));
      Assert.Equal(3*rowNum+1, intInspector.get(inner.getStructFieldData(sub,
          inFields.get(0))));
      Assert.Equal(3*rowNum+2, intInspector.get(inner.getStructFieldData(sub,
          inFields.get(1))));
      rowNum += 1;
    }
    Assert.Equal(3, rowNum);
  ...
  public void testEmptyFile()  {
    ...
    Assert.Equal(null, serde.getSerDeStats());
  }

  [Fact](expected = RuntimeException.class)
  public void testSplitGenFailure() {
    Properties properties = new Properties();
    HiveOutputFormat<?, ?> outFormat = new OrcOutputFormat();
    org.apache.hadoop.hive.ql.exec.FileSinkOperator.RecordWriter writer =
        outFormat.getHiveRecordWriter(conf, testFilePath, MyRow.class, true,
    ...
    try {
      ...
      Assert.Equal(true, e.getMessage().contains("Permission denied"));
      throw e;
    }
  }
  ...
  public void testDefaultTypes()  {
    ...
    SerDeUtils.initializeSerDe(serde, conf, properties, null);
    inspector = (StructObjectInspector) serde.getObjectInspector();
    Assert.Equal("struct<str:string,str2:string>", inspector.getTypeName());
    InputFormat<?,?> in = new OrcInputFormat();
    FileInputFormat.setInputPaths(conf, testFilePath.toString());
    InputSplit[] splits = in.getSplits(conf, 1);
    Assert.Equal(1, splits.length);
    ...
    Assert.Equal(true, reader.next(key, value));
    Assert.Equal("owen", strInspector.getPrimitiveJavaObject(inspector.
        getStructFieldData(value, fields.get(0))));
    Assert.Equal(true, reader.next(key, value));
    Assert.Equal("beth", strInspector.getPrimitiveJavaObject(inspector.
        getStructFieldData(value, fields.get(0))));
    Assert.Equal(true, reader.next(key, value));
    Assert.Equal("laurel", strInspector.getPrimitiveJavaObject(inspector.
        getStructFieldData(value, fields.get(0))));
    Assert.Equal(true, reader.next(key, value));
    Assert.Equal("hazen", strInspector.getPrimitiveJavaObject(inspector.
        getStructFieldData(value, fields.get(0))));
    Assert.Equal(true, reader.next(key, value));
    Assert.Equal("colin", strInspector.getPrimitiveJavaObject(inspector.
        getStructFieldData(value, fields.get(0))));
    Assert.Equal(true, reader.next(key, value));
    Assert.Equal("miles", strInspector.getPrimitiveJavaObject(inspector.
        getStructFieldData(value, fields.get(0))));
    Assert.Equal(false, reader.next(key, value));
    reader.close();
  }
  ...
  public void testVectorization()  {
    ...
    for(int i=0; i < 10; i++) {
      Assert.Equal("checking " + i, i, col0.vector[i]);
    }
    Assert.Equal(false, reader.next(key, value));
  }
   ...
   */
  [Fact]
  public void testVectorizationWithBuckets()  {
    // get the object inspector for MyRow
    StructObjectInspector inspector;
    synchronized (TestOrcFile.class) {
      inspector = (StructObjectInspector)
  ...
  [Fact]
  public void testVectorizationWithAcid()  {
    ...
    for(int i=0; i < 100; i++) {
      Assert.Equal("checking bool " + i, i % 2 == 0 ? 1 : 0,
          booleanColumn.vector[i]);
      Assert.Equal("checking byte " + i, (byte) i,
          byteColumn.vector[i]);
      Assert.Equal("checking short " + i, (short) i, shortColumn.vector[i]);
      Assert.Equal("checking int " + i, i, intColumn.vector[i]);
      Assert.Equal("checking long " + i, i, longColumn.vector[i]);
      Assert.Equal("checking float " + i, i, floatColumn.vector[i], 0.0001);
      Assert.Equal("checking double " + i, i, doubleCoulmn.vector[i], 0.0001);
      Text strValue = new Text();
      strValue.set(stringColumn.vector[i], stringColumn.start[i],
          stringColumn.length[i]);
      Assert.Equal("checking string " + i, new Text(Long.toHexString(i)),
          strValue);
      Assert.Equal("checking decimal " + i, HiveDecimal.create(i),
          decimalColumn.vector[i].getHiveDecimal());
      Assert.Equal("checking date " + i, i, dateColumn.vector[i]);
      long millis = (long) i * MILLIS_IN_DAY;
      millis -= LOCAL_TIMEZONE.getOffset(millis);
      Assert.Equal("checking timestamp " + i, millis * 1000000L,
  ...
  [Fact]
  public void testCombinationInputFormat()  {
    // get the object inspector for MyRow
    StructObjectInspector inspector;
    synchronized (TestOrcFile.class) {
      inspector = (StructObjectInspector)
    ...
    // check split
    Assert.Equal(2, split.getNumPaths());
    Assert.Equal(partDir.toString() + "/0_0", split.getPath(0).toString());
    Assert.Equal(partDir.toString() + "/1_0", split.getPath(1).toString());
    Assert.Equal(length0, split.getLength(0));
    Assert.Equal(outputStream.file.length, split.getLength(1));
    Assert.Equal(0, split.getOffset(0));
    Assert.Equal(0, split.getOffset(1));
    ...
    for(int i=0; i < 20; i++) {
      Assert.Equal(true, reader.next(key, value));
      Assert.Equal(i, ((IntWritable) value.getFieldValue(0)).get());
    }
    Assert.Equal(false, reader.next(key, value));
  }
  ...
  public void testCombinationInputFormatWithAcid()  {
    ...
    Assert.Equal("org.apache.hadoop.hive.ql.io.orc.OrcInputFormat",
        split.inputFormatClassName());
    Assert.Equal("mock:/combinationAcid/p=0/base_0000010/bucket_00000",
        split.getPath().toString());
    Assert.Equal(0, split.getStart());
    Assert.Equal(607, split.getLength());
    split = (HiveInputFormat.HiveInputSplit) splits[1];
    Assert.Equal("org.apache.hadoop.hive.ql.io.orc.OrcInputFormat",
        split.inputFormatClassName());
    Assert.Equal("mock:/combinationAcid/p=0/base_0000010/bucket_00001",
        split.getPath().toString());
    Assert.Equal(0, split.getStart());
    Assert.Equal(629, split.getLength());
    CombineHiveInputFormat.CombineHiveInputSplit combineSplit =
        (CombineHiveInputFormat.CombineHiveInputSplit) splits[2];
    Assert.Equal(BUCKETS, combineSplit.getNumPaths());
    for(int bucket=0; bucket < BUCKETS; ++bucket) {
      Assert.Equal("mock:/combinationAcid/p=1/00000" + bucket + "_0",
          combineSplit.getPath(bucket).toString());
      Assert.Equal(0, combineSplit.getOffset(bucket));
      Assert.Equal(241, combineSplit.getLength(bucket));
    }
  ...
  [Fact]
  public void testSetSearchArgument()  {
    ...
    Assert.Equal(null, colNames[0]);
    Assert.Equal("url", colNames[1]);
    Assert.Equal(null, colNames[2]);
    Assert.Equal("cost", colNames[3]);
    Assert.Equal(null, colNames[4]);
    SearchArgument arg = options.getSearchArgument();
    List<PredicateLeaf> leaves = arg.getLeaves();
    Assert.Equal("cost", leaves.get(0).getColumnName());
    Assert.Equal(PredicateLeaf.Operator.IS_NULL, leaves.get(0).getOperator());
  }

  [Fact]
  public void testSplitElimination()  {
    Properties properties = new Properties();
    properties.setProperty("columns", "z,r");
    properties.setProperty("columns.types", "int:struct<x:int,y:int>");
    StructObjectInspector inspector;
  ...
  [Fact]
  public void testSplitEliminationNullStats()  {
    Properties properties = new Properties();
    StructObjectInspector inspector;
    synchronized (TestOrcFile.class) {
      inspector = (StructObjectInspector)
