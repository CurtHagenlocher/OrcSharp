...
public class TestNewInputOutputFormat {
  ...
  public void testNewInputFormat()  {
    ...
    reader.close();
    Assert.Equal(count, 7500);
    Assert.Equal(lastLine, "{true, 100, 2048, 65536," +
  ...
  public void testNewOutputFormat()  {
    ...
    Assert.Equal(reader.getCompression(), CompressionKind.ZLIB);
    StructObjectInspector soi =
        (StructObjectInspector)reader.getObjectInspector();
    StructTypeInfo ti =
        (StructTypeInfo)TypeInfoUtils.getTypeInfoFromObjectInspector(soi);
    Assert.Equal(((PrimitiveTypeInfo)ti.getAllStructFieldTypeInfos().get(0))
        .getPrimitiveCategory(),
        PrimitiveObjectInspector.PrimitiveCategory.INT);
    Assert.Equal(((PrimitiveTypeInfo)ti.getAllStructFieldTypeInfos().get(1))
        .getPrimitiveCategory(),
    ...
    Assert.Equal(intWritable.get(), firstIntValue);
    Assert.Equal(text.toString(), firstStringValue);
  ...
  public void testNewOutputFormatWithCompression()  {
    ...
    Reader reader = OrcFile.createReader(outputFilePath,
        OrcFile.readerOptions(conf).filesystem(localFs));
    Assert.Equal(reader.getCompression(), CompressionKind.SNAPPY);
  ...
  public void testNewOutputFormatComplex()  {
    Path inputPath = new Path(workDir, "TestOrcFile." +
        testCaseName.getMethodName() + ".txt");
    Path outputPath = new Path(workDir, "TestOrcFile." +
        testCaseName.getMethodName() + ".orc");
    ...
    Assert.Equal(1, converted.get(0));
    Assert.Equal(1, converted.get(1));
    List<Object> list = (List<Object>)converted.get(2);
    Assert.Equal(list.size(), 1);
    Assert.Equal("saving", ((List<Object>)list.get(0)).get(0));
    Assert.Equal(6, ((List<Object>)list.get(0)).get(1));
    Map<String, Integer> map = (Map<String, Integer>)converted.get(3);
    Assert.Equal(map.size(), 1);
    Assert.Equal(map.get("saving"), new Integer(1));
    
    row = rows.next(null);
    converted = (List<Object>)converter.convert(row);
    Assert.Equal(2, converted.get(0));
    Assert.Equal(6, converted.get(1));
    list = (List<Object>)converted.get(2);
    Assert.Equal(list.size(), 6);
    Assert.Equal("breakfast", ((List<Object>)list.get(0)).get(0));
    Assert.Equal(9, ((List<Object>)list.get(0)).get(1));
    map = (Map<String, Integer>)converted.get(3);
    Assert.Equal(map.size(), 11);
    Assert.Equal(map.get("the"), new Integer(2));
    
    row = rows.next(null);
    converted = (List<Object>)converter.convert(row);
    Assert.Equal(3, converted.get(0));
    Assert.Equal(5, converted.get(1));
    list = (List<Object>)converted.get(2);
    Assert.Equal(list.size(), 5);
    Assert.Equal("cold", ((List<Object>)list.get(0)).get(0));
    Assert.Equal(4, ((List<Object>)list.get(0)).get(1));
    map = (Map<String, Integer>)converted.get(3);
    Assert.Equal(map.size(), 13);
    Assert.Equal(map.get("were"), new Integer(3));
  ...
  public void testNewInputFormatPruning()  {
    ...
    Assert.Equal(line, "{null, 1, null, 65536, null, null, null, " +
