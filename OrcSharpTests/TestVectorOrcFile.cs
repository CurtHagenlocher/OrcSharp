...
public class TestVectorOrcFile {
  ...
  public void testTimestamp()  {
    ...
    Assert.Equal(tslist.size(), rows.getRowNumber());
...
//    Assert.Equal(null,stats.getSum());
  ...
  private void compareInner(InnerStruct expect,
    ...
    if (expect == null || actual == null) {
      Assert.Equal(context, null, expect);
      Assert.Equal(context, null, actual);
    } else {
      Assert.Equal(context, expect.int1,
          ((IntWritable) actual.getFieldValue(0)).get());
      Assert.Equal(context, expect.string1, actual.getFieldValue(1));
    }
  }
  ...
  private void compareList(List<InnerStruct> expect,
                           ...
                           String context)  {
    Assert.Equal(context, expect.size(), actual.size());
  ...
  public void testRepeating()  {
    ...
    for(TypeDescription colType: schema.getChildren()) {
      Assert.Equal("count on " + colType.getId(),
          2048, stats[colType.getId()].getNumberOfValues());
      Assert.Equal("hasNull on " + colType.getId(),
    ...
    for(int r=0; r < 1024; ++r) {
      ...
      for(int f=0; f < row.getNumFields(); ++f) {
        Assert.Equal("non-null on row " + r + " field " + f,
    ...
    for(int r=0; r < 1024; ++r) {
      Assert.Equal(true, rows.hasNext());
      row = (OrcStruct) rows.next(row);
      Assert.Equal("row " + r, "48 6f 72 74 6f 6e",
          row.getFieldValue(0).toString());
      Assert.Equal("row " + r, "true", row.getFieldValue(1).toString());
      Assert.Equal("row " + r, "-126", row.getFieldValue(2).toString());
      Assert.Equal("row " + r, "1311768467463790320",
          row.getFieldValue(3).toString());
      Assert.Equal("row " + r, "1.125", row.getFieldValue(4).toString());
      Assert.Equal("row " + r, "9.765625E-4", row.getFieldValue(5).toString());
      Assert.Equal("row " + r, "2011-07-01", row.getFieldValue(6).toString());
      Assert.Equal("row " + r, "2015-10-23 10:11:59.999999999",
          row.getFieldValue(7).toString());
      Assert.Equal("row " + r, "1.234567", row.getFieldValue(8).toString());
      Assert.Equal("row " + r, "Echelon", row.getFieldValue(9).toString());
      Assert.Equal("row " + r, "Juggernaut", row.getFieldValue(10).toString());
      Assert.Equal("row " + r, "Dreadnaugh", row.getFieldValue(11).toString());
      Assert.Equal("row " + r, "{123}", row.getFieldValue(12).toString());
      Assert.Equal("row " + r, "union(1, 1234)",
          row.getFieldValue(13).toString());
      Assert.Equal("row " + r, "[31415, 31415, 31415]",
          row.getFieldValue(14).toString());
      Assert.Equal("row " + r, "{ORC=fast, Hive=fast, LLAP=fast}",
    ...
    for(int r=0; r < 1024; ++r) {
      ...
      Assert.Equal("row " + r, expected.toString(),
          row.getFieldValue(0).toString());
      Assert.Equal("row " + r, r % 2 == 1 ? "true" : "false",
          row.getFieldValue(1).toString());
      Assert.Equal("row " + r, Integer.toString((byte) (r % 255)),
          row.getFieldValue(2).toString());
      Assert.Equal("row " + r, Long.toString(31415L * r),
          row.getFieldValue(3).toString());
      Assert.Equal("row " + r, Float.toString(1.125F * r),
          row.getFieldValue(4).toString());
      Assert.Equal("row " + r, Double.toString(0.0009765625 * r),
          row.getFieldValue(5).toString());
      Assert.Equal("row " + r, new Date(111, 6, 1 + r).toString(),
          row.getFieldValue(6).toString());
      Assert.Equal("row " + r,
          new Timestamp(115, 9, 23, 10, 11, 59 + r, 999999999).toString(),
          row.getFieldValue(7).toString());
      Assert.Equal("row " + r, "1.234567", row.getFieldValue(8).toString());
      Assert.Equal("row " + r, Integer.toString(r),
          row.getFieldValue(9).toString());
      Assert.Equal("row " + r, pad(Integer.toHexString(r), 10),
          row.getFieldValue(10).toString());
      Assert.Equal("row " + r, Integer.toHexString(r * 128),
          row.getFieldValue(11).toString());
      Assert.Equal("row " + r, "{" + Integer.toString(r + 13) + "}",
          row.getFieldValue(12).toString());
      Assert.Equal("row " + r, "union(1, " + Integer.toString(r + 42) + ")",
          row.getFieldValue(13).toString());
      Assert.Equal("row " + r, "[31415, 31416, 31417]",
          row.getFieldValue(14).toString());
  ...
  public void testNonDictionaryRepeatingString()  {
    ...
    for(int r=0; r < 1024; ++r) {
      Assert.Equal(Integer.toString(r * 10001),
  ...
  public void testStructs()  {
    ...
    for(int r=0; r < 1024; ++r) {
      ...
      if (r < 200 || (r >= 400 && r < 600) || r >= 800) {
        Assert.Equal("row " + r, null, inner);
      } else {
        Assert.Equal("row " + r, "{" + r + "}", inner.toString());
      }
    }
    Assert.Equal(false, rows.hasNext());
  }
  ...
  public void testUnions()  {
    ...
    for(int r=0; r < 1024; ++r) {
      ...
      if (r < 200) {
        ...
        Assert.Equal("row " + r, "union(0, " + r +")", inner.toString());
      } else if (r < 400) {
        Assert.Equal("row " + r, "union(1, " + -r +")", inner.toString());
      } else if (r < 600) {
        Assert.Equal("row " + r, null, inner);
      } else if (r < 800) {
        Assert.Equal("row " + r, "union(1, " + -r +")", inner.toString());
  ...
  public void testLists()  {
    ...
    for(int r=0; r < 1024; ++r) {
      ...
      if (r < 200) {
        ...
        Assert.Equal("row " + r, "[" + ((r - 200) * 10) + "]",
            inner.toString());
      } else if (r < 400) {
        Assert.Equal("row " + r, null, inner);
      } else if (r < 500) {
        Assert.Equal("row " + r, "[" + ((r - 300) * 10) + "]",
            inner.toString());
      } else if (r < 600) {
        Assert.Equal("row " + r, null, inner);
      } else if (r < 700) {
        Assert.Equal("row " + r, "[" + (10 * r) + ", " + (10 * (r + 1)) + "]",
            inner.toString());
      } else {
        Assert.Equal("row " + r, null, inner);
      }
  ...
  public void testMaps()  {
    ...
    for(int r=0; r < 1024; ++r) {
      ...
      if (r < 200) {
        ...
        Assert.Equal("row " + r, "{" + (r - 200) + "=" + ((r - 200) * 10) + "}",
            inner.toString());
      } else if (r < 400) {
        Assert.Equal("row " + r, null, inner);
      } else if (r < 500) {
        Assert.Equal("row " + r, "{" + (r - 300) + "=" + ((r - 300) * 10) + "}",
            inner.toString());
      } else if (r < 600) {
        Assert.Equal("row " + r, null, inner);
      } else if (r < 700) {
        Assert.Equal("row " + r, "{" + r + "=" + (r * 10) + ", " +
                (r + 1) + "=" + (10 * (r + 1)) + "}",
            inner.toString());
      } else {
        Assert.Equal("row " + r, null, inner);
