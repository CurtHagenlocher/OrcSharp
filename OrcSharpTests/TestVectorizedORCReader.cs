    ...
    while (vrr.hasNext()) {
      ...
      for (int i = 0; i < batch.size; i++) {
        ...
        for (int j = 0; j < batch.cols.length; j++) {
          ...
          if (a == null) {
            Assert.Assert.Equal(true, !cv.noNulls && cv.isNull[rowId]);
            ...
            Assert.Assert.Equal(temp.toString(), Long.toString(b));
            ...
            long b = ((LongColumnVector) cv).vector[rowId];
            Assert.Assert.Equal(timeInNanoSec.toString(), Long.toString(b));
            ...
            long b = ((LongColumnVector) cv).vector[rowId];
            Assert.Assert.Equal(adt.get().getTime(),
            ...
            Assert.Equal("huh", a.getClass().getName());
          }
        }
      }

      // Check repeating
      Assert.Assert.Equal(false, batch.cols[0].isRepeating);
      Assert.Assert.Equal(false, batch.cols[1].isRepeating);
      Assert.Assert.Equal(false, batch.cols[2].isRepeating);
      Assert.Assert.Equal(true, batch.cols[3].isRepeating);
      Assert.Assert.Equal(false, batch.cols[4].isRepeating);
      Assert.Assert.Equal(false, batch.cols[5].isRepeating);
      Assert.Assert.Equal(false, batch.cols[6].isRepeating);
      Assert.Assert.Equal(false, batch.cols[7].isRepeating);
      Assert.Assert.Equal(false, batch.cols[8].isRepeating);
      Assert.Assert.Equal(false, batch.cols[9].isRepeating);

      // Check non null
      Assert.Assert.Equal(false, batch.cols[0].noNulls);
      Assert.Assert.Equal(false, batch.cols[1].noNulls);
      Assert.Assert.Equal(true, batch.cols[2].noNulls);
      Assert.Assert.Equal(true, batch.cols[3].noNulls);
      Assert.Assert.Equal(false, batch.cols[4].noNulls);
      Assert.Assert.Equal(false, batch.cols[5].noNulls);
      Assert.Assert.Equal(false, batch.cols[6].noNulls);
      Assert.Assert.Equal(false, batch.cols[7].noNulls);
      Assert.Assert.Equal(false, batch.cols[8].noNulls);
      Assert.Assert.Equal(false, batch.cols[9].noNulls);
    }
    Assert.Assert.Equal(false, rr.hasNext());
  }
}
